...
resource "azurerm_app_service" "test" {
  ...
  connection_string {
    name  = "Database"
    type  = "SQLServer"
    value = "Server=tcp:${azurerm_mysql_server.test.fqdn} Database=${azurerm_mysql_server.test.name};User ID=${azurerm_mysql_server.test.administrator_login};Password=${azurerm_mysql_server.test.administrator_login_password};Trusted_Connection=False;Encrypt=True;"
  }
}
